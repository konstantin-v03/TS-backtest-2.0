...
def f_klines_by_datetime(symbol: str, interval: str, start_datetime: datetime = None,
    ...
    while start_unix + interval_to_seconds(interval) < end_unix:
        klines = f_klines(symbol, interval, start_unix,
                          start_unix + interval_to_seconds(interval) * limit - 1
                          if start_unix > interval_to_seconds(interval) * limit
                          else end_unix)
        if df.empty:
            df = klines
        else:
        ...
        start_unix = df.loc[len(df) - 1, UNIX_TIME] + interval_to_seconds(interval)
